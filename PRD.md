...
- [x] Add `percentile(arr, p)` — returns the value at the given percentile (0-100) using linear interpolation. Add to `math_modules/percentile.js`, export from `index.js`, add tests in `test/percentile.test.js`. Test with p=0, p=50 (should match median), p=100, and a mid-range value.
